...
if (isset($_POST['submit'])) {
	...
	if ($count > 0) {
		...
		$query = "INSERT INTO registration (regno, firstName, middleName, lastName, gender, contactno, egycontactno, guardianName, guardianRelation, guardianContactno, emailid, password, corresAddress, corresCity, corresState, corresPincode, pmntAddress, pmntCity, pmnatetState, pmntPincode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$stmt = $mysqli->prepare($query);

		// Assuming the variable values are defined elsewhere in your code
		$stmt->bind_param('issssiississsssisssi', $regno, $fname, $mname, $lname, $gender, $contactno, $emcntno, $gurname, $gurrelation, $gurcntno, $emailid, $password, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode);
		$stmt->execute();
		$stmt->close();

		// $query1 = "insert into  registration(regNo,firstName,middleName,lastName,gender,contactNo,email,password) values(?,?,?,?,?,?,?,?)";
								...
								<div class="panel panel-primary">
									...
									<div class="panel-body">
										<form method="post" action="" class="form-horizontal" onSubmit="return valid();">
											...
											<div class="form-group">
												<label class="col-sm-2 control-label">
													<h4 style="color: green" align="left">Course Details</h4>
												</label>
											</div>
