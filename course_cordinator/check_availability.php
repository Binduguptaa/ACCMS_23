...
if (!empty($_POST["emailid"])) {
	...
	if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
		...
		$result = "SELECT count(*) FROM course_cordinator WHERE emailid=?";
...
if (!empty($_POST["regno"])) {
	...
	$result = "SELECT count(*) FROM course_cordinator WHERE regNo=?";
	$stmt = $mysqli->prepare($result);
...
if (!empty($_POST["oldpassword"])) {
	...
	$result = "SELECT password FROM course_cordinator WHERE password=?";
	$stmt = $mysqli->prepare($result);
