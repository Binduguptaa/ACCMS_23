...
if (isset($_POST['submit'])) {
	...
	$query = "update  course_cordinator set regno=?,firstName=?,middleName=?,lastName=?,gender=?,contactno=?,egycontactno=?,emailid=?,password=?,corresAddress=?,corresCIty=?,corresState=?,corresPincode=?,pmntAddress=?,pmntCity=?,pmnatetState=?,pmntPincode=? where id=?";
	$stmt = $mysqli->prepare($query);

	$rc = $stmt->bind_param('issssiisssssisssii', $regno, $fname, $mname, $lname, $gender, $contactno, $emcntno, $emailid, $password, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode, $id);
	$stmt->execute();
	if ($stmt->affected_rows > 0) {
		// Update successful
		$stmt->close();
		echo "<script>alert('Course has been Updated successfully');</script>";
		header("Location: manage-Ccordinator.php");
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title">Edit course_cordinator </h2>

						<div class="row">
							<div class="col-md-12">
								<div class="panel panel-default">
									<div class="panel-heading">Edit Course Coordinator's</div>
									<div class="panel-body">
										<form method="post" class="form-horizontal">
											<?php
											$id = $_GET['id'];
											$ret = "select * from course_cordinator where id=?";
